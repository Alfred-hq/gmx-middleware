...
if(dayPeriodData){
    ...
    dayPeriodData.cumulativeSize=dayPeriodData.cumulativeSize.plus(event.params.sizeDelta)
...
export function handleUpdatePositionTraderAnalytics(event:UpdatePositionEvent): void{
    let dayPeriodId=getDailyId(event)
    ...
    dayPeriodData.lastOpenPositionAt = trades.status === "Open" ? event.block.timestamp : BigInt.fromString("0");
    dayPeriodData.maxCollateral=dayPeriodData.maxCollateral.gt(event.params.collateral)?dayPeriodData.maxCollateral:event.params.collateral
    dayPeriodData.maxSize=dayPeriodData.maxSize.gt(event.params.size)?dayPeriodData.maxSize:event.params.size
    dayPeriodData.save()
}

export function handleClosePositionTraderAnalytics(event: ClosePositionEvent): void{
    let dayPeriodId=getDailyId(event)
    const trades = Trades.load(
        `${event.transaction.hash.toHexString()}_${event.logIndex.minus(BigInt.fromString('1')).toString()}`
      );
...
export function handleLiquidatePositionTraderAnalytics(event: LiquidatePositionEvent): void{
    ...
    dayPeriodData.totalPositions=dayPeriodData.totalPositions.plus(BigInt.fromString("1"));
    dayPeriodData.lastSettledPositionAt = event.block.timestamp;
    dayPeriodData.openCount = dayPeriodData.openCount.lt(BigInt.fromString("1")) ? BigInt.fromString("0") : dayPeriodData.openCount.minus(BigInt.fromString("1"));
    dayPeriodData.save()
} 

export function loadOrCreateTradeAnalyticsDaily(periodId: i32,account: string):TraderAnalyticsDaily{
